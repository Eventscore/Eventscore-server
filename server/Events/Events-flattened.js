...
async function getSpotify(artistName) {  
    let queryName = artistName.split(' ').join('+');
    let result = await request.get('https://api.spotify.com/v1/search?q=' + queryName + '&type=artist&limit=1');
    return result;
}
    ...
    if (results.length < 10) {
      ...
      .then(function(createdObj) {
        return fetch.Promise.each(createdObj.createdEvents, function(createdEvent) {
          ...
          return fetch.Promise.each(createdEvent.artists, function(artist) {

            // spotifyApi.searchArtists(artist.name)
            createdEvent.artistPromises.push(getSpotify(artist.name));
            return; 
            // .then(function(data) {
            //   var artistBody = {
            //     spotify: {
      ...
      .then(function(createdObj) {
        return fetch.Promise.each(createdObj.createdEvents, function(createdEvent) {
          return fetch.Promise.each(createdEvent.artistPromises, function(artist) {
            createdEvent.rawArtists.push(Promise.resolve(artist));
            return;
      ...
      .then(function(createdObj) {
        return fetch.Promise.each(createdObj.createdEvents, function(createdEvent, outerIndex) {
          // console.log('CREATED EVENT', createdEvent)
          return fetch.Promise.each(createdEvent.rawArtists, function(artist, innerIndex) {
            // var artistBody = {
            //   spotify: {
            ...
            // }
            console.log('ARTIST', artist);
            var test = JSON.stringify(artist);
            console.log('TEST', test);
